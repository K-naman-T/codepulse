...
def create_legacy_server(cp: CodePulse):
    with pytest.warns(DeprecationWarning):
        return create_mcp_server(cp)
...
class TestMCPServer:
    ...
    def test_search_symbols_tool(self, cp: CodePulse):
        server = create_legacy_server(cp)
    ...
    def test_server_error_on_no_data(self, tmp_path: Path):
        from codepulse.config import CodePulseConfig
        config = CodePulseConfig(data_dir=str(tmp_path / ".codepulse"))
        empty_cp = CodePulse(config)
        server = create_legacy_server(empty_cp)
